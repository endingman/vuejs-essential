...
export default {
  ...
  // 指令定义对象的 inserted 钩子在被绑定元素插入父节点时调用，该钩子保证了父节点的存在，因此可以在这里访问父节点
   inserted(el, binding, vnode) {
    const { value, modifiers } = binding
    // 指定当前一系列验证项的父级，我们这里指定为含 data-validator-form 的元素
    const form = el.closest('[data-validator-form]')// closest方法返回被选元素的第一个祖先元素
    // 指定一个按钮来检查所有验证项，我们这里指定为含 type=submit 的元素
    const submitBtn = form ? form.querySelector('[type=submit]') : null// querySelector() 方法返回文档中匹配指定 CSS 选择器的一个元素
